...
class Spawn(commands.Cog):
    ...
    async def spawn_image(self, message):
        ...
        if a > 920 and spawn_flag[message.guild.id] is True:
            ...
            file_path = (
                "./kadlemon/IMG"
                + str(numpy.random.choice(numpy.arange(1, 12), p=weights))
                + ".jpg"
            )
            ...
            await message.channel.send(
                content="A wild Kadle appeared!",
                delete_after=60,
                file=discord.File(fp=file_path, filename="Kadlemon.jpg"),
            )
            ...
            try:
                ...
                await message.channel.send(
                    "Oh no! The wild Kadle fled", delete_after=20
                )
                spawn_flag[message.guild.id] = True
                return
            ...
            await msg.channel.send(
                f"Congratulations {msg.author.mention}! You caught a Level {level} Kadle!",
                delete_after=20,
            )
            ...
            if results is None:
                ...
                results = collection.find_one(
                    {"_id": msg.guild.id, str(msg.author.id): {"$exists": True}}
                )
    ...
    async def spawn(self, ctx, payload=None):
        ...
        await ctx.send(
            f"Kadle will now spawn in the text channel {ctx.message.channel_mentions[0].mention}"
        )
    ...
    async def list(self, ctx, payload=1):
        ...
        results = collection.find_one(
            {"_id": ctx.guild.id, str(ctx.message.author.id): {"$exists": True}}
        )
        if results is None:
        ...
        for key in keys:
            if upper_limit - 10 <= int(key) < upper_limit:
                op += (
                    results[str(ctx.message.author.id)][key]["Name"]
                    + "     #"
                    + str(key)
                    + "      Level "
                    + str(results[str(ctx.message.author.id)][key]["Level"])
                )
            op += "\n"
            last_key = int(key)
        op += "```"
        ...
        e.set_footer(
            text=f"Showing {upper_limit - 10} to {page_limit} of {last_key} Kadles on this page"
        )

        await ctx.send(embed=e, delete_after=300)

    @commands.command(name="select")
    async def select(self, ctx, payload):
        global collection
