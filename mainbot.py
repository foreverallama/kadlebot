...
@bot.command(name="load", help="Loads extensions", usage="kadle.load", hidden=True)
async def load(ctx, extension):
    """Loads a cog"""
    if (
        ctx.message.author.id == 391864327398883329
        and ctx.guild.id == 455640405371912204
    ):
...
@bot.command(
    name="unload", help="Unloads extensions", usage="kadle.unload", hidden=True
)
...
@bot.command(
    name="reload", help="Reloads extensions", usage="kadle.reload", hidden=True
)
...
@bot.command(
    name="clear", help="clears internal state of bot", usage="kadle.clear", hidden=True
)
async def clear():
