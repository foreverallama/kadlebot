...
class MusicBot(commands.Cog):
    ...
    def __init__(self, bot):
        ...
        try:
            ...
            url = (
                "mongodb+srv://kadleBot:"
                + str(MONGO)
                + "@kadlebot-t63y9.mongodb.net/test?retryWrites=true&w=majority"
            )
    ...
    async def leave(self, ctx):
        ...
        if voice and voice.is_connected():
            ...
            await ctx.send("Where would you like me to leave from?")
        ...
        def check_queue():
            ...
            if Queue_infile is True:
                ...
                song_path = os.path.abspath(
                    os.path.realpath(str(ctx.guild.id) + "/Queue") + "/" + first_file
                )
                ...
                if length != 0:
                    ...
                    voice.play(
                        discord.FFmpegPCMAudio(str(ctx.guild.id) + "/song.mp3"),
    ...
    async def queue(self, ctx):
        ...
        await ctx.send(
            f"There are {length} song(s) in the queue. Songs in the list are:\n{song_list}"
        )
    ...
    async def add(self, ctx, *url):
        ...
        if voice is None:
            await ctx.send(
                "Play a song first using kadle.play. Use this command to add songs to the queue"
            )
            return
        ...
        queue_path = os.path.abspath(
            os.path.realpath(str(ctx.guild.id) + "/Queue") + f"/song{q_num}.%(ext)s"
        )
            ...
            try:
                ...
                for file in os.listdir("./" + str(ctx.guild.id) + "/Queue"):
                    if file.endswith(".mp3"):
                        ...
                        song_list_queue.setdefault(ctx.guild.id, []).append(song_title)
                        os.rename(
                            str(ctx.guild.id) + "/Queue/" + file,
                            str(ctx.guild.id) + "/Queue/song" + str(q_num) + ".mp3",
                        )
    ...
    async def remove(self, ctx, track_no):
        ...
        if Queue_infile is False:
            ...
            song_there = os.path.isfile(
                "./" + str(ctx.guild.id) + "/Queue/song" + str(track_no - 1) + ".mp3"
            )
            if song_there:
                os.remove(
                    "./"
                ...
                removed_value = song_list_queue.setdefault(ctx.guild.id, []).pop(
                    track_no - 1
                )
                await ctx.send(
                    f"The track {removed_value} has been removed from the queue"
                )
        ...
        if any(word in payload.upper() for word in available_opts):
            ...
            if not results:
                collection.insert_one(
                    {"_id": ctx.guild.id, "music_search": payload.upper()}
                )
            else:
                results = collection.update_one(
                    {"_id": ctx.guild.id}, {"$set": {"music_search": payload.upper()}}
                )
