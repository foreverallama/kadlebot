...
class Love:
    ...
    async def date(self, ctx):
        ...
        if percentage >= 100:
            await self.bot.send_message(ctx.message.channel, "You have met your one true soulmate!")
        elif percentage>=80:
            await self.bot.send_message(ctx.message.channel, "You should definitely go on a date. My calculations say you are " + str(percentage) + "% compatible")
        elif percentage>=50:
            await self.bot.send_message(ctx.message.channel, "You could give it a shot. My calculations say you have a " + str(percentage) + "% compatibility")
        elif percentage>=20:
            await self.bot.send_message(ctx.message.channel, "You can try but don't expect much. You are only " + str(percentage) + "% compatible")
        else:
            await self.bot.send_message(ctx.message.channel, "No one would ever date a waste of sperm like you")
    ...
    async def eqtest(self, ctx):
        await self.bot.send_message(ctx.message.channel, ctx.message.author.mention + " I will ask you 5 yes-no questions to determine your personality. "
        ...
        msg = await self.bot.wait_for_message(timeout=60, author = ctx.message.author, channel = ctx.message.channel, content='kadle.start')
        if msg: 
            ...
            answers = []
            def check(msg):
                if str(msg.content) == "kadle.yes":
                    return 'YES'
                elif str(msg.content) == "kadle.no":
                    return 'NO'
                        
            for i in range(0,5):
                await self.bot.send_message(ctx.message.channel, current_questions[i])
                msg = await self.bot.wait_for_message(timeout=60, author = ctx.message.author, channel = ctx.message.channel, check = check)
                if msg:
                    answers.append(str(msg.content))
                else:
                    await self.bot.send_message(ctx.message.channel, ctx.message.author.mention + " Answer madalla andre time waste madbeda goobe")
            ...
            if question_bank == 0:
                if answers[0] == 'kadle.yes' and answers[1] == 'kadle.yes' and answers[2] == 'kadle.yes' and answers[3] == 'kadle.yes' and answers[4] == 'kadle.yes':
                    ...
                    await self.bot.send_message(ctx.message.channel, ctx.message.author.mention + msg)
